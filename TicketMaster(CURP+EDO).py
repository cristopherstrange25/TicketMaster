...
# Función para validar el usuario (CURP + 2 caracteres adicionales del estado)
def validar_usuario(username):
    patron = r'^[A-Z]{4}\d{6}[A-Z]{1}[A-Z]{2}[A-Z0-9]{3}[A-Z]{2}\d{2}$'
    if not re.fullmatch(patron, username):
        return False, "El usuario debe seguir el formato correcto, sin minusculas ni carateres especiales: [4L] + [6D] + [1L] + [2L] + [3LN] + [2L Estado] + [2D]"
    return True, ""
...
class LoginScreen:
    def __init__(self, root):
        ...
        self.label_username = tk.Label(root, text="Usuario (CURP + Estado):")
...
class RegisterScreen:
    def __init__(self, root):
        ...
        self.label_new_username = tk.Label(root, text="Nuevo Usuario (CURP + Estado):")
    ...
    def register(self):
        ...
        valido, mensaje = validar_usuario(username)
        if not valido:
